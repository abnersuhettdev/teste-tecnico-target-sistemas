...
function faturamento(){
    alert("Resposta no console!")
        ...
        function calcularFaturamento(mes){
            ...
            const diasMediaSuperior = mes.filter((dias)=>{
                return dias.valor > (total/30)      
            })
            
            diasMediaSuperior.forEach((dia)=> dias.push(dia.dia))
        
            console.log("O menor Faturamento foi de: R$", menorFaturamento);
        
            console.log("O maior Faturamento foi de: R$",maiorFaturamento);
        
            console.log("A média de faturamento é de: R$", total/30 )
            console.log("Número de dias em que o valor de faturamento foi superior a média mensal:", dias.join(', ') )
        }
